...
export default function App() {
  ...
  // 초기화
  useEffect(() => {
    const newToken = parseSpotifyToken();
  ...
  const saveToSpotify = async () => {
    if (!spToken || !playlist) return;
    setSaving(true); setSaveMsg("");
    try {
      const user = spUser || await getSpotifyUser(spToken);
      // 곡 검색
      const uris = [];
      for (const t of playlist.tracks) {
        const uri = await spotifySearch(spToken, `${t.title} ${t.artist}`);
        if (uri) uris.push(uri);
      }
      if (uris.length === 0) throw new Error("검색된 곡이 없어요");
      // 플레이리스트 생성
      const plId = await createSpotifyPlaylist(spToken, user.id, playlist.title, playlist.description);
      await addTracksToPlaylist(spToken, plId, uris);
